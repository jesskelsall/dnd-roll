...
test('returns a function', async () => {
  ...
  expect(rollDoubleDice('advantage')).toMatchJoiSchema(functionSchema)
  expect(rollDoubleDice('disadvantage')).toMatchJoiSchema(functionSchema)
})

test('returns a function that returns a DoubleRoll', async () => {
